...
class StabilityProbe(BaseProbe):
    ...
    def analyze(self, evidence_list: list[HTTPEvidence]) -> FindingResult:
        """基于证据列表分析会话稳定性 — 只分析 baseline 证据."""
        facts: list[str] = []
        inferences: list[str] = []

        # 只分析 baseline 类型的证据
        baseline_evidence = [e for e in evidence_list if e.evidence_type == "baseline"]
        total = len(baseline_evidence)

        if total == 0:
            return self._result(
                ProbeStatus.WARN,
                Severity.INFO,
                facts=["无可用的 baseline 证据"],
        ...
        # 检查状态码一致性
        status_codes = {e.http_status for e in baseline_evidence if e.http_status is not None}
