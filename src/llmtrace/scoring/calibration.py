...
# ---------------------------------------------------------------------------
# Claimed model gap (§13–§14)
# ---------------------------------------------------------------------------


class AmbiguousClaimedModelError(CalibrationError):
    """The claimed model matches multiple distinct reference identities."""

    def __init__(self, message: str) -> None:
        super().__init__(message, error_code="AMBIGUOUS_CLAIMED_MODEL")


def compute_claimed_model_gap(
    *,
    claimed_model: str,
    identities: list[ReferenceIdentity],
    curves: CalibrationCurveBundle,
    candidate_profile: CapabilityProfile,
) -> ClaimedModelGap | None:
    """Compute the capability gap against the claimed model's reference.

    Matching is strict (§14): the claimed ``model_id`` must match exactly one
    distinct reference identity (case-sensitive, no fuzzy/similar-name
    matching).  Measurement-condition compatibility is already guaranteed by
    the ReferenceSet compatibility gate — every member shares the same suite,
    scoring policy, generation config, and adapter as the candidate.

    Returns:
        ``ClaimedModelGap`` when exactly one compatible reference identity
        matches the claimed model; ``None`` when no identity matches (caller
        reports "gap unavailable").  Raises :class:`AmbiguousClaimedModelError`
        when more than one distinct identity matches — an ambiguous claim
        must fail closed rather than silently pick one.
    """
    matches = [identity for identity in identities if identity.model_id == claimed_model]
    if not matches:
        return None
    if len(matches) > 1:
        providers = ", ".join(sorted({m.provider_id for m in matches}))
        raise AmbiguousClaimedModelError(
            f"claimed model '{claimed_model}' matches {len(matches)} distinct reference identities "
            f"(providers: {providers}); refusing to pick one for the gap"
        )

    reference = matches[0]
    reference_total = curves.total_curve.calibrate(reference.total_raw)

    dimension_gaps: list[ClaimedModelDimensionGap] = []
    candidate_by_dimension = {d.dimension: d for d in candidate_profile.dimensions}
    for dimension, curve in curves.dimension_curves.items():
        candidate_dim = candidate_by_dimension.get(dimension)
        if candidate_dim is None or candidate_dim.calibrated_score is None:
            continue
        raw = reference.dimension_raws.get(dimension)
        if raw is None:
            continue
        reference_score = curve.calibrate(raw)
        dimension_gaps.append(
            ClaimedModelDimensionGap(
                dimension=dimension,
                candidate_score=candidate_dim.calibrated_score,
                reference_score=reference_score,
                delta=candidate_dim.calibrated_score - reference_score,
            )
        )

    candidate_total = candidate_profile.calibrated_total_score
    if candidate_total is None:  # pragma: no cover — gap is only called on calibrated profiles
        raise CandidateIncompatibleError("claimed model gap requires a calibrated candidate profile")

    return ClaimedModelGap(
        claimed_model_id=claimed_model,
        reference_provider_id=reference.provider_id,
        reference_model_id=reference.model_id,
        candidate_total_score=candidate_total,
        reference_total_score=reference_total,
        total_delta=candidate_total - reference_total,
        dimension_gaps=tuple(dimension_gaps),
    )
